...
class LibShadingBlur : Box
{
    ...
    private Mat ImageProcessing(Mat sourceImg, (int kernelX, int kernelY, int selectMenu, int noiseCut) Param)
    {
        Mat dstImg = new();
        sourceImg = sourceImg.CvtColor(ColorConversionCodes.BGR2GRAY);
        Cv2.Blur(sourceImg, dstImg, new OpenCvSharp.Size(Param.kernelX, Param.kernelY));
    ...
    private void SetParam((int kernelX, int kernelY, int selectMenu, int noiseCut) Param)
    {
    ...
    // Public Function
    // ****************************************
    public (int kernelX, int kernelY, int selectMenu, int noiseCut) GetParam()
    {
        return ((int)_scaleX.Get(), (int)_scaleY.Get(), _optMenu.GetIndex(), (int)_scaleRemove.Get());
    }

    public Mat Run(Mat sourceImg, (int kernelX, int kernelY, int selectMenu, int noiseCut) Param = default)
    {
